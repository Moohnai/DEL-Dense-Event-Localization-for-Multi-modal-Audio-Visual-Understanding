...
class UnAV100Dataset(Dataset):
    ...
    def _load_json_db(self, json_file):
        ...
        # add _video to the json_db keys
        json_db = {key + '_video': value for key, value in json_db.items()}
    ...
    def label_points_single_video(self, concat_points, gt_segment, gt_label):
        ...
        # gt_segment : N (#Events) x 2
        # gt_label : N (#Events) x 1
        num_pts = concat_points.shape[0] # 441
        num_gts = gt_segment.shape[0]
    ...
    def __getitem__(self, idx):
        try:
            ...
            feats = torch.load(filename, weights_only=True)
            ...
            # T x C -> C x T
            feats_visual = feats.transpose(0, 1)

            #load audio features
            filename_audio = os.path.join(self.feat_folder,
                                self.file_prefix + video_item['id'].replace('_video', '_audio') + self.file_ext)
            feats_audio = torch.load(filename_audio, weights_only=True)
            feats_audio = feats_audio[::self.downsample_rate, :]
            feats_audio = feats_audio.transpose(0, 1)
            ...
            if video_item['segments'] is not None:
                ...
                # keep all segments only cut the end time based on the feature length
                new_segments = []
                new_labels = []
                for i, seg in enumerate(segments):
                    if seg[1] < feats['visual'].shape[1] and seg[0] < feats['visual'].shape[1]:
                        new_segments.append(seg)
                        new_labels.append(video_item['labels'][i])
                if len(new_segments) == 0:
                    return None
                segments = torch.stack(new_segments)
            ...
            if video_item['id'] == '1_0T2ZIjWnw_video':
                a=0
            ...
            if data_dict['gt_offsets'].shape != (441, self.num_classes, 2):
                print('Error: ', data_dict['gt_offsets'].shape, points[0].shape[0], self.num_classes, 2)
                print('Video ID: ', data_dict['video_id'])
                return None
