...
class MaskedResnetBlock(nn.Module):
    def __init__(
        self,
        *,
        in_channels: int,
        ...
        **kwargs,
        ...
        self.nin_shortcut = (
            MaskedConv1D(in_channels, out_channels, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        h = self.norm1(h)
        h = nonlinearity(h)
        h, mask = self.conv1(h, mask)

        h = self.norm2(h)
        h = nonlinearity(h)
        h = self.dropout(h)
        h, mask = self.conv2(h, mask)

        if isinstance(self.nin_shortcut, MaskedConv1D):
            x, mask = self.nin_shortcut(x, mask)
        else:
...
class MaskedMHCA(nn.Module):
    ...
    def __init__(
        ...
        self.query_conv = MaskedConv1D(
            in_channels=self.n_embd, out_channels=self.n_embd, kernel_size=kernel_size,
